...
#include "ui_zaspirecontroldialog.h"
...
std::list<std::string> ZAspireControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZAspireControlDialog::listMints;

ZAspireControlDialog::ZAspireControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZAspireControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZAspireControlDialog::~ZAspireControlDialog()
{
    delete ui;
}

void ZAspireControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAspireControlDialog::updateList()
{
...
void ZAspireControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAspireControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZAspire_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAspireControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZAspireControlDialog::GetSelectedMints()
{
...
void ZAspireControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
