...
#include "zaspirecontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zASPIRE ought to be enough for anybody." - Bill Gates, 2017
    ui->zASPIREpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzASPIRESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzASPIRE->setEnabled(false);
        ui->pushButtonMintzASPIRE->setToolTip(tr("zASPIRE is currently disabled due to maintenance."));

        ui->pushButtonSpendzASPIRE->setEnabled(false);
        ui->pushButtonSpendzASPIRE->setToolTip(tr("zASPIRE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zASPIREpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzASPIRE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zASPIRE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zASPIRE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zASPIRE in ") +
...
void PrivacyDialog::on_pushButtonSpendzASPIRE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zASPIRE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zASPIRE
        sendzASPIRE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zASPIRE
    sendzASPIRE();
}

void PrivacyDialog::on_pushButtonZAspireControl_clicked()
{
    ZAspireControlDialog* zASPIREControl = new ZAspireControlDialog(this);
    zASPIREControl->setModel(walletModel);
    zASPIREControl->exec();
}

void PrivacyDialog::setZAspireControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzASPIRESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzASPIRE()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Aspire Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zASPIREpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zASPIRE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ASPIRE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zASPIRE</b>";
    ...
    // use mints from zASPIRE selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZAspireControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZAspireControlDialog::GetSelectedMints();
    }
    ...
    // Clear zaspire selector in case it was used
    ZAspireControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zASPIRE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Aspire, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zASPIRE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zASPIRE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zASPIRE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zASPIRE "));
    ui->labelzASPIREAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzASPIRESyncStatus->setVisible(fShow);
}
